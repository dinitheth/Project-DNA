...
describe('pull request impact state', () => {
  it('accepts only the active base/head request identity', () => {
    ...
    const second = reducePullRequestImpactState(first, { type: 'request', requestId: 2, baseSha: 'c'.repeat(40), headSha: 'd'.repeat(40) });
    expect(reducePullRequestImpactState(second, { type: 'message', message: response(1, 'a', 'b') })).toBe(second);
    expect(reducePullRequestImpactState(second, { type: 'message', message: response(2, 'c', 'e') })).toBe(second);
  });
  ...
  it('suppresses cancelled results and clears on workspace unavailability', () => {
    ...
    expect(reducePullRequestImpactState(cancelled, { type: 'message', message: response(3, 'a', 'b') })).toBe(cancelled);
    expect(reducePullRequestImpactState(cancelled, { type: 'message', message: ExtensionMessageSchema.parse({ type: 'analysisUnavailable', rootPath: null }) })).toEqual({ ...initialPullRequestImpactState, requestId: 3 });
  });
  ...
  it('rejects malformed persisted state and restores loading requests safely', () => {
    ...
    expect(restorePullRequestImpactState({ pullRequestImpact: { ...loading, baseSha: 'HEAD' } })).toEqual(initialPullRequestImpactState);
  });
});
...
function response(requestId: number, base: string, head: string) {
  return ExtensionMessageSchema.parse({
    type: 'pullRequestImpactResult', requestId, baseSha: base.repeat(40), headSha: head.repeat(40), mergeBaseSha: null, result: null, error: 'Unavailable',
  });
}
