...
export type PullRequestImpactStatus =
  'idle' | 'editing' | 'loading' | 'ready' | 'error' | 'cancelled';
...
export function reducePullRequestImpactState(
  ...
  if (action.type === 'open')
    return state.visible ? state : { ...state, visible: true, status: 'editing' };
  if (action.type === 'close')
    return { ...initialPullRequestImpactState, requestId: state.requestId };
  if (action.type === 'request') {
    return {
      visible: true,
      status: 'loading',
      requestId: action.requestId,
      baseSha: action.baseSha,
  ...
  if (!message.result) {
    return {
      ...state,
      status: 'error',
      mergeBaseSha: message.mergeBaseSha,
      error: message.error ?? 'PR impact is unavailable.',
    };
  }
  const parsed = PullRequestImpactDataSchema.safeParse(message.result);
  if (
    !parsed.success ||
    parsed.data.baseCommitSha !== state.baseSha ||
    parsed.data.headCommitSha !== state.headSha ||
    parsed.data.mergeBaseSha !== message.mergeBaseSha
  )
    return state;
  return {
    ...state,
...
export function restorePullRequestImpactState(candidate: unknown): PullRequestImpactState {
  ...
  if (
    !isSafeRequestId(state.requestId) ||
    typeof state.visible !== 'boolean' ||
    !isStatus(state.status) ||
    !isNullableSha(state.baseSha) ||
    !isNullableSha(state.headSha) ||
    !isNullableSha(state.mergeBaseSha) ||
    (state.result !== null &&
      state.result !== undefined &&
      !PullRequestImpactDataSchema.safeParse(state.result).success)
  )
    return initialPullRequestImpactState;
  ...
  return {
    ...
    mergeBaseSha: state.mergeBaseSha ?? null,
    result: state.result ? PullRequestImpactDataSchema.parse(state.result) : null,
...
function isStatus(value: unknown): value is PullRequestImpactStatus {
  return ['idle', 'editing', 'loading', 'ready', 'error', 'cancelled'].includes(String(value));
}
